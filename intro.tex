...
\begin{definition}[Polar set] \label{def:polar_set}
    For any convex set $\Cscr \subseteq \Re^n$, its polar set is given by 
...
\begin{definition}[Support function] \label{def:support-fn}
    Given a set $\Ascr \subseteq \Re^n$, the support function $\sigma\As:\Re^n\to\Re$ is defined as  
...
\section{Duality in optimization} \label{sec:dual-opt}
...
\section{Structured optimization} \label{sec:struc-opt}
